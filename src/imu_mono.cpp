...
public:
      ...
      inertial_ba1_(false), inertial_ba2_(false), tImage_(0.0)
  {
    ...
    if (sensor_type_param == "monocular") {
      ...
      settings_file_path =
        std::string(PROJECT_PATH) + "/config/Monocular/TUM-VI.yaml";
      ...
      settings_file_path = std::string(PROJECT_PATH) +
                           "/config/Monocular-Inertial/TUM-VI.yaml";
    } else {
    ...
    odom_publisher_ = 
      create_publisher<nav_msgs::msg::Odometry>("orb_odom", 10);
    ...
    timer = create_wall_timer(
      100ms, std::bind(&ImuMono::timer_callback, this),
      timer_callback_group_);
...
private:
  ...
  void timer_callback()
  {
    ...
    if (orb_slam3_system_->isImuInitialized()) {
      ...
      rclcpp::Time dataset_time(total_nanoseconds, RCL_SYSTEM_TIME);
